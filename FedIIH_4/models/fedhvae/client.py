...
class HVAE(nn.Module):
    ...
    def sampling(self, z_mu_n, z_logvar_n, z_mu_e, z_logvar_e, z_mu_n1, z_logvar_n1, z_mu_e1, z_logvar_e1):
        ...
        return z_n, z_e, z_n1, z_e1

    def decode(self, z_n, z_e, z_n1, z_e1):

        z = torch.cat((z_n, z_e, z_n1, z_e1), dim=1)  
    ...
    def loss_function(self, batch, z_mu_n, z_logvar_n, z_mu_e, z_logvar_e, z_mu_n1, z_logvar_n1, z_mu_e1, z_logvar_e1,
                      Alpha_mu, Beta_mu, Alpha_mu1, Beta_mu1, edge_logits,
                      **kwargs) -> dict:
        ...
        l_elbo = torch.mean(
            log_pmu_Alpha + extra_kl_Alpha + log_pmu_Beta + extra_kl_Beta + log_pmu_Alpha1 + extra_kl_Alpha1 + log_pmu_Beta1 + extra_kl_Beta1 + logpx_z + kl_structure + kl_semantic + kl_structure1 + kl_semantic1)
    ...
    def train(self):
        for ep in range(self.args.n_eps):
            ...
            for _, batch in enumerate(self.loader.pa_loader):
                ...
                if self.args.dataset in ['Minesweeper', 'Tolokers', 'Questions']:  
                    train_lss = F.binary_cross_entropy_with_logits(y_hat[batch.train_mask].view(-1),
                                                                   batch.y[batch.train_mask].view(-1))
                else:
        ...
        for ep in range(self.args.n_eps):
            ...
            for _, batch in enumerate(self.loader.pa_loader):
                ...
                self.z_mu_n, self.z_logvar_n, self.z_mu_e, self.z_logvar_e, self.z_mu_n1, self.z_logvar_n1, self.z_mu_e1, self.z_logvar_e1 = self.model.encode_for_HVAE(
                    batch)

                z_n, z_e, z_n1, z_e1 = self.vae.sampling(self.z_mu_n, self.z_logvar_n, self.z_mu_e, self.z_logvar_e, self.z_mu_n1, self.z_logvar_n1, self.z_mu_e1, self.z_logvar_e1)

                edge_logits = self.vae.decode(z_n, z_e, z_n1, z_e1)

                loss = self.vae.loss_function(batch, self.z_mu_n, self.z_logvar_n, self.z_mu_e, self.z_logvar_e, self.z_mu_n1, self.z_logvar_n1, self.z_mu_e1, self.z_logvar_e1,
