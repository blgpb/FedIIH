...
class HVAE(nn.Module):
    ...
    def sampling(self, z_mu_n, z_logvar_n, z_mu_e, z_logvar_e, z_mu_n1, z_logvar_n1, z_mu_e1, z_logvar_e1, z_mu_n2,
                 z_logvar_n2, z_mu_e2, z_logvar_e2, z_mu_n3, z_logvar_n3, z_mu_e3, z_logvar_e3):
        ...
        return z_n, z_e, z_n1, z_e1, z_n2, z_e2, z_n3, z_e3

    def decode(self, z_n, z_e, z_n1, z_e1, z_n2, z_e2, z_n3, z_e3):


        z = torch.cat((z_n, z_e, z_n1, z_e1, z_n2, z_e2, z_n3, z_e3), dim=1)  # (270, 512)
    ...
    def loss_function(self, batch, z_mu_n, z_logvar_n, z_mu_e, z_logvar_e, z_mu_n1, z_logvar_n1, z_mu_e1, z_logvar_e1,
                      ...
                      Alpha_mu, Beta_mu, Alpha_mu1, Beta_mu1, Alpha_mu2, Beta_mu2, Alpha_mu3, Beta_mu3, edge_logits,
                      **kwargs) -> dict:
        ...
        l_elbo = torch.mean(
            log_pmu_Alpha + extra_kl_Alpha + log_pmu_Beta + extra_kl_Beta + log_pmu_Alpha1 + extra_kl_Alpha1 + log_pmu_Beta1 + extra_kl_Beta1 + log_pmu_Alpha2 + extra_kl_Alpha2 + log_pmu_Beta2 + extra_kl_Beta2 + log_pmu_Alpha3 + extra_kl_Alpha3 + log_pmu_Beta3 + extra_kl_Beta3 + logpx_z + kl_structure + kl_semantic + kl_structure1 + kl_semantic1 + kl_structure2 + kl_semantic2 + kl_structure3 + kl_semantic3)
    ...
    def train(self):
        ...
        for ep in range(self.args.n_eps):
            ...
            for _, batch in enumerate(self.loader.pa_loader):
                ...
                self.z_mu_n, self.z_logvar_n, self.z_mu_e, self.z_logvar_e, self.z_mu_n1, self.z_logvar_n1, self.z_mu_e1, self.z_logvar_e1, self.z_mu_n2, self.z_logvar_n2, self.z_mu_e2, self.z_logvar_e2, self.z_mu_n3, self.z_logvar_n3, self.z_mu_e3, self.z_logvar_e3 = self.model.encode_for_HVAE(
                    batch)

                z_n, z_e, z_n1, z_e1, z_n2, z_e2, z_n3, z_e3 = self.vae.sampling(self.z_mu_n, self.z_logvar_n,
                                                                                 self.z_mu_e,
                                                                                 self.z_logvar_e,
                ...
                edge_logits = self.vae.decode(z_n, z_e, z_n1, z_e1, z_n2, z_e2, z_n3, z_e3)
