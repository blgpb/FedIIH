...
class HVAE(nn.Module):
    def __init__(self, hidden_dim, nfeat, args, g_id):
        ...
        self.latent_factor_nums = 1  
        self.latent_factor_dims = hidden_dim  
        self.routit = args.n_routit  
        self.gpu_id = g_id


        self.mu_Beta = nn.Parameter(torch.empty(self.latent_factor_dims),
                                    requires_grad=True)  
    ...
    def sampling(self, z_mu_n, z_logvar_n):
    ...
    def decode(self, z_n):
        ...
        A_pred = torch.matmul(z_n, z_n.t())

        return A_pred

    def loss_function(self, batch, z_mu_n, z_logvar_n,
                      Beta_mu, edge_logits,
                      **kwargs) -> dict:
    ...
    def train(self):
        ...
        for ep in range(self.args.n_eps):
            ...
            for _, batch in enumerate(self.loader.pa_loader):
                ...
                self.z_mu_n, self.z_logvar_n = self.model.encode_for_HVAE(batch)

                z_n = self.vae.sampling(self.z_mu_n, self.z_logvar_n)

                edge_logits = self.vae.decode(z_n)

                loss = self.vae.loss_function(batch, self.z_mu_n, self.z_logvar_n,
                                              self.Beta_mu,
                                              edge_logits)
