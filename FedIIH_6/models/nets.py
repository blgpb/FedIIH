...
class DisentangledGNN(nn.Module):
    ...
    def encode_for_HVAE(self, data):
        ...
        z_mu_n, z_mu_e, z_mu_n1, z_mu_e1, z_mu_n2, z_mu_e2 = torch.chunk(mean, chunks=self.latent_factor_nums, dim=1)

        logstd = x
        for conv in self.gnn_logstddev:
            logstd = self._dropout(F.leaky_relu(conv(logstd, edge_index)))
        z_logvar_n, z_logvar_e, z_logvar_n1, z_logvar_e1, z_logvar_n2, z_logvar_e2 = torch.chunk(logstd,
                                                                                                 chunks=self.latent_factor_nums,
                                                                                                 dim=1)

        return z_mu_n, z_logvar_n, z_mu_e, z_logvar_e, z_mu_n1, z_logvar_n1, z_mu_e1, z_logvar_e1, z_mu_n2, z_logvar_n2, z_mu_e2, z_logvar_e2
