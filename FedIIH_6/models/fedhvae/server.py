...
class Server(ServerModule):
    ...
    def aggregate_sp(self, local_weights, ratio_semantic, ratio_structure, ratio_semantic1, ratio_structure1, ratio_semantic2, ratio_structure2):
        ...
        for name, params in aggr_theta.items():
            if name == 'clf.bias':
                ...
                aggregated_structure = np.split(aggregated_structure_model, indices_or_sections=6, axis=1)[1]

                aggregated_structure_model1 = np.sum(
                    [theta[name] * ratio_structure1[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_structure1 = np.split(aggregated_structure_model1, indices_or_sections=6, axis=1)[3]

                aggregated_structure_model2 = np.sum(
                    [theta[name] * ratio_structure2[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_structure2 = np.split(aggregated_structure_model2, indices_or_sections=6, axis=1)[5]

                aggregated_semantic_model = np.sum(
                    [theta[name] * ratio_semantic[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic = np.split(aggregated_semantic_model, indices_or_sections=6, axis=1)[0]

                aggregated_semantic_model1 = np.sum(
                    [theta[name] * ratio_semantic1[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic1 = np.split(aggregated_semantic_model1, indices_or_sections=6, axis=1)[2]

                aggregated_semantic_model2 = np.sum(
                    [theta[name] * ratio_semantic2[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic2 = np.split(aggregated_semantic_model2, indices_or_sections=6, axis=1)[4]

                aggregated_model_cat = np.concatenate(
                    (aggregated_semantic, aggregated_structure, aggregated_semantic1, aggregated_structure1, aggregated_semantic2, aggregated_structure2), axis=1)

                aggr_theta[name] = aggregated_model_cat
            elif name == 'pca.bias':
                aggregated_structure_model = np.sum(
                    [theta[name] * ratio_structure[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_structure = np.split(aggregated_structure_model, indices_or_sections=6, axis=0)[1]

                aggregated_structure_model1 = np.sum(
                    [theta[name] * ratio_structure1[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_structure1 = np.split(aggregated_structure_model1, indices_or_sections=6, axis=0)[3]

                aggregated_structure_model2 = np.sum(
                    [theta[name] * ratio_structure2[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_structure2 = np.split(aggregated_structure_model2, indices_or_sections=6, axis=0)[5]

                aggregated_semantic_model = np.sum(
                    [theta[name] * ratio_semantic[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic = np.split(aggregated_semantic_model, indices_or_sections=6, axis=0)[0]

                aggregated_semantic_model1 = np.sum(
                    [theta[name] * ratio_semantic1[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic1 = np.split(aggregated_semantic_model1, indices_or_sections=6, axis=0)[2]

                aggregated_semantic_model2 = np.sum(
                    [theta[name] * ratio_semantic2[j] for j, theta in enumerate(local_weights)], 0)
                aggregated_semantic2 = np.split(aggregated_semantic_model2, indices_or_sections=6, axis=0)[4]

                aggregated_model_cat = np.concatenate((aggregated_semantic, aggregated_structure, aggregated_semantic1, aggregated_structure1, aggregated_semantic2, aggregated_structure2), axis=0)
